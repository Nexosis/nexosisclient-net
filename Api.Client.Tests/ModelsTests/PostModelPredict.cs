...
namespace Api.Client.Tests.ModelsTests
{
    public class PostModelPredict : NexosisClient_TestsBase
    {
        ...
        public async Task WillSendValuesForPrediction()
        {
            ...
            await target.Models.Predict(modelId, new List<Dictionary<string, string>> { new Dictionary<string, string> { ["column"] = "value" } });
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(async () => await target.Models.Predict(Guid.NewGuid(), null));
