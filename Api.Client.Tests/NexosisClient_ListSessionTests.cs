...
namespace Api.Client.Tests
{
    public class NexosisClient_ListSessionTests
    {
        ...
        public NexosisClient_ListSessionTests()
        {
            ...
            target = new NexosisClient("abcdefg", "https://nada.nexosis.com/not-here", new NexosisClient.HttpClientFactory(handler));
        }

        [Fact]
        public async Task FormatsPropertiesForListSessions()
        {
            var result = await target.ListSessions("alpha", "zulu", DateTimeOffset.Parse("2017-01-01"), DateTimeOffset.Parse("2017-01-11"));
        ...
        {
            var result = await target.ListSessions();
