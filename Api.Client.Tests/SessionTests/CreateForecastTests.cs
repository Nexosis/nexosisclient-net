...
namespace Api.Client.Tests.SessionTests
{
    public class CreateForecastTests : NexosisClient_TestsBase
    {
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(async () => await target.Sessions.CreateForecast((IEnumerable<DataSetRow>) null, "", DateTimeOffset.MinValue, DateTimeOffset.MaxValue, ResultInterval.Day));
