...
namespace Api.Client.Tests.SessionTests
{
    public class CreateImpactTests : NexosisClient_TestsBase
    {
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(async () => await target.Sessions.AnalyzeImpact((IEnumerable<DataSetRow>) null, "event", "", DateTimeOffset.MinValue, DateTimeOffset.MaxValue, ResultInterval.Day));
