...
namespace Api.Client.Tests.DataSetTests
{
    public class GetTests : NexosisClient_TestsBase
    {
        ...
        {
            var result = await target.DataSets.GetDataSet("test");
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await target.DataSets.GetDataSet(""));
        ...
        {
            var result = await target.DataSets.GetDataSet("test", 10, 10, DateTimeOffset.Parse("2017-01-01"), DateTimeOffset.Parse("2017-01-31"), new []{ "test1", "test2" });
